...
class Auth
{
	...
	public static function validate_token($token = null, $refresh = false)
	{
		...
		try {
			...
			// custom filter, by Neumann Valle
			// it would create an user if 
			// secret matches with nextjs enviroment var
			$token = apply_filters('graphql_jwt_nextjs_secret', $token, self::get_secret_key(), self::get_token_issued(), self::get_token_expiration(), JWT::$supported_algs);
